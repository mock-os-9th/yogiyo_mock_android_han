...
public class OrderCompleteService {
    ...
    void getOrderList() {  //서버통신부분
        final OrderCompleteRetrofitInterfaces orderCompleteRetrofitInterface = getRetrofit4().create(OrderCompleteRetrofitInterfaces.class);
        orderCompleteRetrofitInterface.getOrerList().enqueue(new Callback<OrderListResponse>() {  //비동기 호출
            @Override
            public void onResponse(Call<OrderListResponse> call, Response<OrderListResponse> response) {
                final OrderListResponse orderListResponse = response.body(); //response = 서버에서 api통신을 통해 얻은 json..?
                ...
                ApplicationClass.mOrderListViewDataList.get(0).orderDate = orderListResponse.orderListResult.get(0).createdAt;
                ApplicationClass.mOrderListViewDataList.get(0).storeName = orderListResponse.orderListResult.get(0).orderStoreName;

                mOrderCompleteActivityView.OrderListSuccess(orderListResponse.orderListResult.get(0).createdAt);
